...
const SkillItemDialog = memo(
  ({ skill, isOpen, onClose }: SkillItemDialogProps) => {
    const tags = useMemo(
      () => (skill?.tags ? skill.tags.split(",").map((tag) => tag.trim()) : []),
      [skill],
    );

    return (
      <Dialog isOpen={isOpen} onClose={onClose} title="Skill Details">
        {skill && (
