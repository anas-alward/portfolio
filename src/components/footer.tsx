...
const Footer = () => {
  const { data: profile } = useSupabaseSingleQuery<Profile | null>({
    key: ["profile"],
    table: "profile",
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col items-center gap-2 text-[13px] text-muted-foreground/50 font-medium">
        ...
        <p className="max-w-75 text-center leading-relaxed">
          Design inspired by{" "}
          <a
            className="underline hover:text-foreground/60 transition-colors"
            href="https://basics.framer.media/"
            target="_blank"
          ...
          <a
            className="underline hover:text-foreground/60 transition-colors"
            href="https://www.framer.com/@samar/"
            target="_blank"
            rel="noopener noreferrer"
          >
            Samar Jamil
